...
// Prototype des fonctions ajoutées
int lireEntier();
...
void afficherTerrain(int nbLignes, int nbColonnes)
/*
Affichage du contour du terrain
...
int recupererTouche()
/*
Teste si le joueur a appuyé sur une touche
...
int calculerDirectionTouche(int touche)
/*
Calcul de la direction correspondant à une touche ou -1
...
void positionAleatoire(int nbLignes, int nbColonnes, int &posX, int &posY)
/*
Calcul d'une position aléatoire sur le terrain
...
void deplacerSerpentI(int direction, int &posX, int &posY)
/*
Déplacement du serpent dans la direction
...
int saisirNiveau()
/*
Niveau de difficulté avec tests de validation d'entrée
...
/*
Génération des nbSouris aléatoirement sur le terrain
...
/*
Déplacement le serpent dans la direction
...
bool testerCollision(int posX, int posY,
	int sourisX[], int sourisY[], int &nbSouris)
	/*
	Test si (posX, posY) est la position d'une souris. Si oui, retirer
	...
	*/
{
	/*Iteration du tableau du suris et on compare si les coordonnes de la tete de la serpent sont egales pour retourne un valeur "vrai" si 
...
/*Fonction tres similar au ce du souris pour tester si la serpent mange son propre corps*/
bool testerCollisionSerpent(int serpentX[], int serpentY[], int indiceTete, int indiceQueue)
{
	int teteSerpentX = serpentX[indiceTete];
	int teteSerpentY = serpentY[indiceTete];
	...
	do
	{
		/*Tour dedans le tableau de la serpent pour verifier si l'indice de la tete est le meme que ce de la queue*/
		...
		/*Finalement, vérification si la tete de la serpent est dans un indice passe et pourtant elle a mangé son propre corps on return vrai */
...
void dessinerSerpent(int serpentX[], int serpentY[], int indiceTete, int indiceQueue) {
	...
	cout << "O";
/*Quand la serpent bouge on efface (avec l'impression d'un space) le derniere index, mais si on est dans l'index 0 on doit aller à
la fin du tableau*/
	if (indiceQueue == 0){
